...
_main PROC
	...
	fld cztery
	 ...
	 fcomi st(0), st(1)			; rozkaz FCOMI - oba porównywane operandy musza byæ podane na stosie 
								; koprocesora
	 fstp st(0)					; usuniecie zera z wierzcho³ka stosu
	 ...
	 fst st(3)					; przechowanie obliczonej wartoœci
				; sytuacja na stosie: ST(0) = sqrt(b^2 - 4 * a * c), ST(1) = b, 
				; ST(2) = 2 * a, ST(3) = sqrt(b^2 - 4 * a * c)
	 
	 fchs						; zmiana znaku
	 fsub st(0), st(1)			; obliczenie -sqrt(delta) - b
